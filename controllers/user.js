...
exports.addApiRequest = (req, res, next) => {
  const formatYmd = date => date.toISOString().slice(0, 10);
  const current_day = formatYmd(new Date()); 
  console.log(current_day);

  const sir_name = req.headers["sirName"] || 'default';
  const first_name = req.headers["name"] || 'default';
  const email = req.headers["email"] || 'default';
  const phone_number = req.headers["phoneNumber"] || 0;
  const mineralName = req.headers["mineralName"] || 'default';
  const amount = req.headers["amount"]||  0;
  const price = req.headers["price"] || 0;
  const country = req.headers["country"] || 'default';
  const date = current_day || 'default';
  const status = 'pending';
  const description = req.headers["description"] || 'default';
  const req_type = req.headers["type"] || 'default';
  const license_number = req.headers["license_number"]  ||'default';
  const mineral_type = req.headers["mineral_type"] || 'default';
  ...
  // })
  // .catch(err => console.log('error database' + err));
  return res.status(200).send({message: 'success adding request'});
}
